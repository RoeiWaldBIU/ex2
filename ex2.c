...
#include <stdio.h>
// define 10 as subtruct and used well we want to subtruct the left digit from a number and to extract the number with modolo.
# define subtruct 10
int main() {
    int key;
    int flag = 1;
        ...
        switch (key) {
            ...
            // if a number is one digit it is harmony
                if (counter == 1)
                    ...
                    if (counter % 2 == 1)
                        number /= subtruct;
                    // sum the digits from the right to the middle.
                    for (int i = 0; i < (counter / 2); i++) {
                        sumR += number % subtruct;
                        number /= subtruct;
                    }
                    if (sumL == sumR)
                        printf("This number is balanced and brings harmony!\n");
